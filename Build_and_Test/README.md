...
| ADC |Solder ADC and remaining components.  Can omit V_Ref buffer for now.  Everything else should now be populated. | Clock at 100 Msps. Verilog code to capture ADC readings and allow MCU to read via SPI. | OK, but ADC A ~10 mV offset.  ADC B ~5mV offset.  Also silk screen polarity for ADC A is inverted.|
